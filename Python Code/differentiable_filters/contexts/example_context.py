...
class ExampleContext(base.BaseContext):
    ...
    def get_loss(self, label, prediction):
        ...
        # Calculate NEES
        nees = tf.reduce_mean(tf.linalg.matmul(tf.linalg.matmul(diff[:, :, None, :], 
                                                            tf.linalg.inv(covars)), 
                                            diff[:, :, :, None]), axis=[1, 2, 3])
        ...
        # Compute the error in the predicted observations 
        ...
        if self.loss == 'nees':
            total_loss = tf.reduce_mean(nees)
        elif self.loss == 'nll':
            total_loss = tf.reduce_mean(likelihood)
        elif self.loss == 'mse':
            total_loss = tf.reduce_mean(total_mse)
        elif self.loss == 'mixed':
            total_loss = (tf.reduce_mean(total_mse) + tf.reduce_mean(likelihood)) / 2.
        elif self.loss == 'obs':
            total_loss = tf.reduce_mean(mse_obs)
        elif self.loss == 'hybrid':
            total_loss = (tf.reduce_mean(likelihood) + tf.reduce_mean(nees)) / 2.
        ...
        metric_names = ['nll', 'mse', 'euclidean_distance',
                        ...
                        'correlation r visibility', 'nees', 'wd']
