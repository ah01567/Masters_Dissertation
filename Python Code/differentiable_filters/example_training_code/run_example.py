...
def run_example(filter_type, loss, out_dir, batch_size, hetero_q, hetero_r,
    ...
    """
    if use_gpu:
        # limit tensorflow's GPU memory consumption
    ...
    # Define the path to save or load model weights
    weight_path = "c:\\users\\pc\\desktop\\"
    model_weights_path = os.path.join(weight_path, 'model_weights_nees.h5')
    ...
    # Run a dummy forward pass to create the model's variables
    dummy_observations = np.zeros((batch_size, 10, image_size, image_size, 3)) 
    dummy_initial_state = np.zeros((batch_size, model.dim_x))
    dummy_noise = np.zeros((batch_size, model.dim_x))
    
    # Initialize the step counter to a valid value
    tf.summary.experimental.set_step(0)

    # Create the dummy input as a tuple of the three expected inputs
    dummy_input = (dummy_observations, dummy_initial_state, dummy_noise)
    ...
    # Check if model weights exist, if so load them
    if os.path.exists(model_weights_path):
        print(f"Loading model weights from {model_weights_path} ...")
        model.load_weights(model_weights_path)
        print("Weights loaded successfully. Proceeding to testing...")
    else:
        ...
        final_training_loss = float(loss_value)
    ...
    # Test the trained or loaded model on the held-out data
...
def evaluate(model, dataset, batch_size):
    ...
    None.
    """
    # Define the desktop path for exporting files
    desktop_path = "c:\\users\\pc\\desktop\\"
    ground_truth_file = os.path.join(desktop_path, 'ground_truth.txt')
    innovation_sequence_file = os.path.join(desktop_path, 'innovation_sequence.txt')
    prediction_errors_file = os.path.join(desktop_path, 'prediction_errors.txt')
    Q_file = os.path.join(desktop_path, 'Q.txt')
    R_file = os.path.join(desktop_path, 'R.txt')

    outputs = {}
    nis_values = []

    # Iterate over the batches of the test set.
    for step, (x_batch, y_batch) in enumerate(dataset):
        ...
        # This ensures that Q and R are calculated or updated as needed
        loss_value, metrics, metric_names = model.context.get_loss(y_batch, out)

        # Extract relevant data
        true_states = y_batch[0].numpy()  # Ground truth states
        estimated_states = out[2].numpy()  # Estimated states from the model
        z = out[4].numpy()  # Predicted observations
        innovation = true_states[:, :, :2] - z  # Innovation sequence
        prediction_errors = np.abs(true_states - estimated_states)  # Prediction errors
        Q_matrices = out[6].numpy()  # Process noise covariance Q
        R_matrices = out[5].numpy()  # Observation noise covariance R

        # Calculate the NIS for each time step in the batch
        for i in range(batch_size):
            for t in range(true_states.shape[1]):
                innov = innovation[i, t]
                R_inv = np.linalg.inv(R_matrices[i, t])
                nis_value = np.dot(np.dot(innov.T, R_inv), innov)
                nis_values.append(nis_value)

        # Export all data for each time step in a single loop
        with open(ground_truth_file, 'a') as f_gt, \
             open(innovation_sequence_file, 'a') as f_innov, \
             open(prediction_errors_file, 'a') as f_pe, \
             open(Q_file, 'a') as f_Q, \
             open(R_file, 'a') as f_R:

            for i in range(batch_size):
                for t in range(true_states.shape[1]):
                    # Write ground truth for this time step
                    f_gt.write(' '.join(map(str, true_states[i, t])) + '\n')
                    
                    # Write innovation sequence for this time step
                    f_innov.write(' '.join(map(str, innovation[i, t])) + '\n')
                    
                    # Write prediction errors for this time step
                    f_pe.write(' '.join(map(str, prediction_errors[i, t])) + '\n')
                    
                    # Write Q matrix values (4 per line)
                    Q_flat = Q_matrices[i, t].flatten()
                    for j in range(0, len(Q_flat), 4):
                        f_Q.write(' '.join(map(str, Q_flat[j:j+4])) + '\n')
                    
                    # Write R matrix values (2 per line)
                    R_flat = R_matrices[i, t].flatten()
                    for k in range(0, len(R_flat), 2):
                        f_R.write(' '.join(map(str, R_flat[k:k+2])) + '\n')

        # Collect metrics to verify correctness during evaluation
        if step == 0:
    ...
    # Compute the mean NIS over all time steps
    mean_nis = np.mean(nis_values)
    print(f"Mean NIS value: {mean_nis:.4f}")

    print("\nEvaluation Complete. Data exported to:")
    print(f" - {ground_truth_file}")
    print(f" - {innovation_sequence_file}")
    print(f" - {prediction_errors_file}")
    print(f" - {Q_file}")
    print(f" - {R_file}")
...
def main():
    ...
    parser.add_argument('--loss', dest='loss', type=str,
                        default='nees', choices=['nll', 'mse', 'mixed', 'nees', 'hybrid'],
